...
if ! modprobe 9pnet_virtio 2>/dev/null \
    ...
    echo "WARNING: could not load 9pnet_virtio module; mount may still work if built-in." >&2
fi
...
mount -t 9p -o trans=virtio,version=9p2000.L hostshare /mnt/host || {
    ...
    # Brief delay so the error is visible on serial console before powering off.
    sleep 2
    echo o > /proc/sysrq-trigger
}
